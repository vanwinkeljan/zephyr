...
static int flash_native_posix_init(struct device *dev)
{
	...
	if (data->fd == -1) {
		LOG_ERR("Failed to open flash device file %s: %s",
			data->flash_path, strerror(errno));
		return -EIO;
	}

	if (ftruncate(data->fd, config->flash_size) == -1) {
		LOG_ERR("Failed to resize flash device file %s: %s",
			data->flash_path, strerror(errno));
		return -EIO;
	}
	...
	if (data->flash == MAP_FAILED) {
		LOG_ERR("Failed to mmap flash device file %s: %s",
			data->flash_path, strerror(errno));
		return -EIO;
	}
...
#if defined(CONFIG_FLASH_PAGE_LAYOUT)
	.layout = { .pages_count = (CONFIG_FLASH_SIZE * 1024) /
