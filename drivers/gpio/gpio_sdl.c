...
static void gpio_sdl_isr(void *arg)
{
	...
	gpio_fire_callbacks(&data->callbacks, dev, callbacks);
}
...
static bool gpio_sdl_handle_toggle_switch(u32_t pin, struct gpio_sdl_data *data,
					  SDL_Event *event)
{
	bool pin_state = ((data->pin_state & BIT(pin)) != 0);
	bool key_state = ((data->key_state & BIT(pin)) != 0);

	if (event->type == SDL_KEYUP) {
		data->key_state &= ~BIT(pin);
		return pin_state;
	}
...
static bool gpio_sdl_handle_impulse_switch(u32_t pin,
					   ...
					   SDL_Event *event)
{
	bool key_pressed = ((data->key_state & BIT(pin)) != 0);

	if (key_pressed && (event->type == SDL_KEYDOWN)) {
		data->pin_state &= ~BIT(pin);
		return false;
	}
...
static void gpio_sdl_handle_irq_generation(u32_t pin,
					   struct gpio_sdl_data *data,
					   const struct gpio_sdl_config *cfg,
					   bool prev_pin_state,
					   bool new_pin_state)
{
	...
	if ((flags & GPIO_INT_EDGE) == GPIO_INT_EDGE) {
		...
		if ((flags & GPIO_INT_ACTIVE_HIGH) == GPIO_INT_ACTIVE_HIGH) {
			trigger_irq = (new_pin_state == true);
		} else {
			trigger_irq = (new_pin_state == false);
		}
	}
				      ...
				      SDL_Event *event)
{
	...
	bool prev_pin_state = ((data->pin_state & BIT(pin)) != 0);
